...
#include <format>
#include <spdlog/spdlog.h>  // Ensure spdlog is included for logging
...
namespace plugin::QuestHooks {
    ...
    void SetCurrentStageID_Hook(RE::TESQuest* quest, std::uint16_t stage) {
        try {
            ...
            spdlog::error("Exception in SetCurrentStageID_Hook: {}", e.what());
        } catch (...) {
            spdlog::error("Unknown exception in SetCurrentStageID_Hook");
        }
    }
    ...
    void InstallQuestStageHook() {
        try {
            ...
            spdlog::error("Exception in InstallQuestStageHook: {}", e.what());
        } catch (...) {
            spdlog::error("Unknown exception in InstallQuestStageHook");
        }
    }
    ...
    void SetObjectiveCompleted_Hook(RE::TESQuest* quest, std::uint32_t objectiveIndex, bool completed) {
        try {
            ...
            spdlog::error("Exception in SetObjectiveCompleted_Hook: {}", e.what());
        } catch (...) {
            spdlog::error("Unknown exception in SetObjectiveCompleted_Hook");
        }
    }
    ...
    void InstallObjectiveCompletedHook() {
        try {
            ...
            spdlog::error("Exception in InstallObjectiveCompletedHook: {}", e.what());
        } catch (...) {
            spdlog::error("Unknown exception in InstallObjectiveCompletedHook");
        }
    }
}  // namespace plugin::QuestHooks